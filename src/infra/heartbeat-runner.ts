...
function resolveActiveHoursTimezone(cfg: OpenSoulConfig, raw?: string): string {
...
export type HeartbeatRunner = {
  stop: () => void;
  updateConfig: (cfg: OpenSoulConfig) => void;
};

function hasExplicitHeartbeatAgents(cfg: OpenSoulConfig) {
  const list = cfg.agents?.list ?? [];
  return list.some((entry) => Boolean(entry?.heartbeat));
}

export function isHeartbeatEnabledForAgent(cfg: OpenSoulConfig, agentId?: string): boolean {
...
function resolveHeartbeatAgents(cfg: OpenSoulConfig): HeartbeatAgent[] {
  const list = cfg.agents?.list ?? [];
...
export function resolveHeartbeatPrompt(cfg: OpenSoulConfig, heartbeat?: HeartbeatConfig) {
  return resolveHeartbeatPromptText(heartbeat?.prompt ?? cfg.agents?.defaults?.heartbeat?.prompt);
}

function resolveHeartbeatAckMaxChars(cfg: OpenSoulConfig, heartbeat?: HeartbeatConfig) {
  return Math.max(
    0,
...
export function startHeartbeatRunner(opts: {
  ...
  const updateConfig = (cfg: OpenSoulConfig) => {
    if (state.stopped) {
      return;
    }
    const now = Date.now();
