...
- **CI 与安装稳定性**：`install-smoke` 工作流固定使用 Node 22，统一公开安装脚本地址为 `opensoul.ai`，并为 Docker 安装冒烟链路补充 `curl` 重试与超时控制。
- **发布前验证**：发布与提交流程文档补充了 `pnpm github:validate`、`pnpm protocol:check` 和安装冒烟测试的适用场景。
