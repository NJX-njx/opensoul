...
OpenSoul extension for Zalo Personal Account messaging via [zca-cli](https://zca-cli.dev).
...
```bash
opensoul channels login --channel zalouser              # Login via QR
opensoul channels login --channel zalouser --account work
...
```bash
opensoul directory self --channel zalouser
opensoul directory peers list --channel zalouser --query "name"
opensoul directory groups list --channel zalouser --query "work"
opensoul directory groups members --channel zalouser --group-id <id>
```
...
```bash
# Text
opensoul message send --channel zalouser --target <threadId> --message "message"

# Media (URL)
opensoul message send --channel zalouser --target <threadId> --message "caption" --media-url "https://example.com/img.jpg"
```

### Listener

The listener runs inside the Gateway when the channel is enabled. For debugging,
use `opensoul channels logs --channel zalouser` or run `zca listen` directly.
...
```bash
opensoul channels login --channel zalouser --account work
opensoul message send --channel zalouser --account work --target <id> --message "Hello"
