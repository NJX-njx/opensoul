...
`opensoul.invoke --each`) without adding new OpenSoul code per workflow.
...
This extension depends on OpenSoul internal modules (the embedded agent runner).
It is intended to ship as a **bundled** OpenSoul extension (like `lobster`) and
be enabled via `plugins.entries` + tool allowlists.

It is **not** currently designed to be copied into
`~/.opensoul/extensions` as a standalone plugin directory.
