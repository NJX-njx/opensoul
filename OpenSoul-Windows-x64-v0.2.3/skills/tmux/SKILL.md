...
```bash
SOCKET_DIR="${OPENSOUL_TMUX_SOCKET_DIR:-${OPENCLAW_TMUX_SOCKET_DIR:-${TMPDIR:-/tmp}/opensoul-tmux-sockets}}"
mkdir -p "$SOCKET_DIR"
...
- Use `OPENSOUL_TMUX_SOCKET_DIR` (legacy `OPENCLAW_TMUX_SOCKET_DIR` also supported).
