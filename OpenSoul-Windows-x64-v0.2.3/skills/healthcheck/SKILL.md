---
name: healthcheck
description: Host security hardening and risk-tolerance configuration for OpenSoul deployments. Use when a user asks for security audits, firewall/SSH/update hardening, risk posture, exposure review, OpenSoul cron scheduling for periodic checks, or version status checks on a machine running OpenSoul (laptop, workstation, Pi, VPS).
---

# OpenSoul Host Hardening

## Overview

Assess and harden the host running OpenSoul, then align it to a user-defined risk tolerance without breaking access. Use OpenSoul security tooling as a first-class signal, but treat OS hardening as a separate, explicit set of steps.
...
- Never claim OpenSoul changes the host firewall, SSH, or OS updates; it does not.
...
5. OpenSoul gateway status and bind address.
...
9. OS automatic security updates status.
   Note: these are not blocking items, but are highly recommended, especially if OpenSoul can access sensitive data.
...
### 2) Run OpenSoul security audits (read-only)

As part of the default read-only checks, run `opensoul security audit --deep`. Only offer alternatives if the user requests them:

1. `opensoul security audit` (faster, non-probing)
2. `opensoul security audit --json` (structured output)

Offer to apply OpenSoul safe defaults (numbered):

1. `opensoul security audit --fix`

Be explicit that `--fix` only tightens OpenSoul defaults and file permissions. It does not change host firewall, SSH, or OS update policies.
...
### 3) Check OpenSoul version/update status (read-only)

As part of the default read-only checks, run `opensoul update status`.
...
- Credential hygiene notes (location of OpenSoul creds, prefer disk encryption)
...
- OpenSoul security audit (re-run)
...
After OpenSoul install or first hardening pass, run at least one baseline audit and version check:

- `opensoul security audit`
- `opensoul security audit --deep`
- `opensoul update status`

Ongoing monitoring is recommended. Use the OpenSoul cron tool/CLI to schedule periodic audits (Gateway scheduler). Do not create scheduled tasks without explicit approval. Store outputs in a user-approved location and avoid secrets in logs.
...
1. “Do you want me to schedule periodic audits (e.g., daily/weekly) via `opensoul cron add`?”

If the user says yes, ask for:

- cadence (daily/weekly), preferred time window, and output location
- whether to also schedule `opensoul update status`
...
Before creating, `opensoul cron list` and match on exact `name`. If found, `opensoul cron edit <id> ...`.
If not found, `opensoul cron add --name <name> ...`.

Also offer a periodic version check so the user can decide when to update (numbered):

1. `opensoul update status` (preferred for source checkouts and channels)
2. `npm view opensoul version` (published npm version)

## OpenSoul command accuracy

Use only supported commands and flags:

- `opensoul security audit [--deep] [--fix] [--json]`
- `opensoul status` / `opensoul status --deep`
- `opensoul health --json`
- `opensoul update status`
- `opensoul cron add|list|runs|run`

Do not invent CLI flags or imply OpenSoul enforces host firewall/SSH policies.
...
Follow the durable-memory prompt format used by OpenSoul compaction:
