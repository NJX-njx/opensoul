...
If the page fails to load, start the Gateway first: `opensoul gateway`.
...
you revoke it with `opensoul devices revoke --device <id> --role <role>`. See
...
- Config: view/edit `~/.opensoul/opensoul.json` (`config.get`, `config.set`)
...
(`tailscale-user-login`) when `gateway.auth.allowTailscale` is `true`. OpenSoul
...
OpenSoul **blocks** Control UI connections without device identity.
...
- `token` is stored in sessionStorage; `password` is kept in memory only.
