...
- **Default: restart gateway after each modification** — After any code change that affects the gateway (including `src/gateway/`, `ui/`, config, etc.), automatically restart the gateway so the user can verify changes without manual steps.

### UI Workflow (after modifying `ui/`)

After any change to `ui/` (styles, components, app-render, etc.):

1. Run `pnpm ui:build` to update `dist/control-ui`
2. Stop processes on port 19001 (if gateway is running)
3. Restart gateway: `$env:OPENSOUL_SKIP_CHANNELS="1"; $env:OPENSOUL_GATEWAY_TOKEN="dev-token"; node scripts/run-node.mjs --dev gateway` (PowerShell)
