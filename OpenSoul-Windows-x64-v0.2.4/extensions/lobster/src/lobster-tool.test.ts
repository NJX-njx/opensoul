...
import type { OpenSoulPluginApi, OpenSoulPluginToolContext } from "../../../src/plugins/types.js";
import { createLobsterTool } from "./lobster-tool.js";

async function writeFakeLobsterScript(scriptBody: string, prefix = "opensoul-lobster-plugin-") {
...
function fakeApi(overrides: Partial<OpenSoulPluginApi> = {}): OpenSoulPluginApi {
  return {
    id: "lobster",
    name: "lobster",
    source: "test",
...
function fakeCtx(overrides: Partial<OpenSoulPluginToolContext> = {}): OpenSoulPluginToolContext {
  return {
    config: {},
    workspaceDir: "/tmp",
    agentDir: "/tmp",
...
describe("lobster plugin tool", () => {
  ...
  it("can be gated off in sandboxed contexts", async () => {
    const api = fakeApi();
    const factoryTool = (ctx: OpenSoulPluginToolContext) => {
      if (ctx.sandboxed) {
        return null;
      }
