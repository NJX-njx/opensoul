...
## OpenSoul Runtime Mapping

- **Task tool** in the upstream spec == OpenSoul `sessions_spawn`
- **File I/O** == OpenSoul `read`/`write`
- **Remote fetch** == OpenSoul `web_fetch` (or `exec` with curl when POST is required)
