...
export function extractMSTeamsPollVote(
  ...
  const pollId =
    readNestedString(value, ["opensoulPollId"]) ??
    readNestedString(value, ["pollId"]) ??
    readNestedString(value, ["opensoul", "pollId"]) ??
    readNestedString(value, ["opensoul", "poll", "id"]) ??
    readNestedString(value, ["data", "opensoulPollId"]) ??
    readNestedString(value, ["data", "pollId"]) ??
    readNestedString(value, ["data", "opensoul", "pollId"]);
  if (!pollId) {
    return null;
  }
    ...
    actions: [
      {
        ...
        title: "Vote",
        data: {
          opensoulPollId: pollId,
          pollId,
        },
        msteams: {
          type: "messageBack",
          text: "opensoul poll vote",
          displayText: "Vote recorded",
          value: { opensoulPollId: pollId, pollId },
        },
      },
    ],
  };
