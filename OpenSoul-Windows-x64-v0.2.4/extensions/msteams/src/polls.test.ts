import type { PluginRuntime } from "opensoul/plugin-sdk";
...
const runtimeStub = {
  state: {
    resolveStateDir: (env: NodeJS.ProcessEnv = process.env, homedir?: () => string) => {
      ...
      return path.join(resolvedHome, ".opensoul");
    },
  },
...
describe("msteams polls", () => {
  ...
  it("stores and records poll votes", async () => {
    const home = await fs.promises.mkdtemp(path.join(os.tmpdir(), "opensoul-msteams-polls-"));
