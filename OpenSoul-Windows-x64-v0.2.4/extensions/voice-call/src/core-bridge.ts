...
function resolveOpenSoulRoot(): string {
  if (coreRootCache) {
    return coreRootCache;
  }
  const override = process.env.OPENSOUL_ROOT?.trim();
  if (override) {
  ...
  throw new Error("Unable to resolve core root. Set OPENSOUL_ROOT to the package root.");
}
...
async function importCoreExtensionAPI(): Promise<{
  ...
  const distPath = path.join(resolveOpenSoulRoot(), "dist", "extensionAPI.js");
